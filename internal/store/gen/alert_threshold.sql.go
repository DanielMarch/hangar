...
const countEnabledSubscriptionsForRoutePaths = `-- name: CountEnabledSubscriptionsForRoutePaths :many
SELECT p.upstream_path::text AS upstream_path,
  ...
  FROM unnest($1::text[]) AS p(upstream_path)
...
type CountEnabledSubscriptionsForRoutePathsRow struct {
	UpstreamPath         string
	EnabledSubscriptions int64
	RouteCatalogued      bool
}
...
func (q *Queries) CountEnabledSubscriptionsForRoutePaths(ctx context.Context, upstreamPaths []string) ([]CountEnabledSubscriptionsForRoutePathsRow, error) {
	rows, err := q.db.Query(ctx, countEnabledSubscriptionsForRoutePaths, upstreamPaths)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []CountEnabledSubscriptionsForRoutePathsRow
	for rows.Next() {
		var i CountEnabledSubscriptionsForRoutePathsRow
		if err := rows.Scan(&i.UpstreamPath, &i.EnabledSubscriptions, &i.RouteCatalogued); err != nil {
			return nil, err
		}
...
const listExpiringCorporationContracts = `-- name: ListExpiringCorporationContracts :many
SELECT c.owner_id       AS corporation_id,
 ...
 WHERE c.owner_kind = 'corporation'
   ...
   AND c.date_expired <= now() + $1::interval
 ORDER BY c.date_expired, c.owner_id, c.contract_id
`

type ListExpiringCorporationContractsRow struct {
	CorporationID int64
	ContractID    int64
	Type          string
	Title         *string
...
func (q *Queries) ListExpiringCorporationContracts(ctx context.Context, within time.Duration) ([]ListExpiringCorporationContractsRow, error) {
	rows, err := q.db.Query(ctx, listExpiringCorporationContracts, within)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListExpiringCorporationContractsRow
	for rows.Next() {
		var i ListExpiringCorporationContractsRow
		if err := rows.Scan(
			&i.CorporationID,
			&i.ContractID,
			&i.Type,
...
const listInactiveCorporationMembers = `-- name: ListInactiveCorporationMembers :many
SELECT m.corporation_id,
 ...
 WHERE m.logoff_date IS NOT NULL
   AND m.logoff_date < now() - $1::interval
 ORDER BY m.logoff_date, m.corporation_id, m.character_id
`

type ListInactiveCorporationMembersRow struct {
	CorporationID int64
	CharacterID   int64
	LogonDate     *time.Time
...
// the inactive list.
func (q *Queries) ListInactiveCorporationMembers(ctx context.Context, inactiveFor time.Duration) ([]ListInactiveCorporationMembersRow, error) {
	rows, err := q.db.Query(ctx, listInactiveCorporationMembers, inactiveFor)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListInactiveCorporationMembersRow
	for rows.Next() {
		var i ListInactiveCorporationMembersRow
		if err := rows.Scan(
			&i.CorporationID,
			&i.CharacterID,
			&i.LogonDate,
...
const listStarbasesLowOnFuel = `-- name: ListStarbasesLowOnFuel :many
SELECT d.corporation_id,
 ...
 WHERE jsonb_array_length(d.fuels) > 0
   AND q.fuel_quantity <= $1::bigint
 ORDER BY q.fuel_quantity, d.corporation_id, d.starbase_id
`

type ListStarbasesLowOnFuelRow struct {
	CorporationID int64
	StarbaseID    int64
	SystemID      int32
	State         *string
...
func (q *Queries) ListStarbasesLowOnFuel(ctx context.Context, below int64) ([]ListStarbasesLowOnFuelRow, error) {
	rows, err := q.db.Query(ctx, listStarbasesLowOnFuel, below)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListStarbasesLowOnFuelRow
	for rows.Next() {
		var i ListStarbasesLowOnFuelRow
		if err := rows.Scan(
			&i.CorporationID,
			&i.StarbaseID,
			&i.SystemID,
...
const listStructuresLowOnFuel = `-- name: ListStructuresLowOnFuel :many
...
SELECT s.corporation_id,
 ...
 WHERE s.fuel_expires IS NOT NULL
   AND s.fuel_expires <= now() + $1::interval
 ORDER BY s.fuel_expires, s.corporation_id, s.structure_id
`

type ListStructuresLowOnFuelRow struct {
	CorporationID int64
	StructureID   int64
	TypeID        int32
	SystemID      int32
...
func (q *Queries) ListStructuresLowOnFuel(ctx context.Context, within time.Duration) ([]ListStructuresLowOnFuelRow, error) {
	rows, err := q.db.Query(ctx, listStructuresLowOnFuel, within)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListStructuresLowOnFuelRow
	for rows.Next() {
		var i ListStructuresLowOnFuelRow
		if err := rows.Scan(
			&i.CorporationID,
			&i.StructureID,
			&i.TypeID,
