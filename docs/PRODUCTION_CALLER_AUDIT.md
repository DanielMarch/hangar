...
| **`esi_ledger_divergence` spikes to 40-55 under per-bucket concurrency — a THIRD instance of "operands describing different moments"** | Measured on the live installation immediately after 20.3's fix landed, by sampling `/metrics` every 3s for two minutes: `char-industry` 46, `char-social` 55, `char-detail` 51, `corp-industry` 50, `corp-member` 42, `corp-detail` 43, each persisting 3-6s. At steady state (~4.3 jobs/s, one character) the same 10-sample window reads max **1**. Mechanism: `local_remaining` is computed LIVE at scrape time while `server_remaining` is a snapshot from the last response that carried the header, so the gauge reports (requests settled since that reading) × 2 whenever several routes in the SAME rate-limit group for the SAME character settle between two reconciles. Not caused by 20.3 — `BackgroundRateLimitMax` returns 0 for every group but char-notification, so those buckets' behaviour is byte-for-byte unchanged | **NOT fixed. Recorded for 20.4, and it is a Gate 1.3 blocker.** 20.2 fixed the >one-window case with `readingIsCurrent`; the freshness rule cannot catch a sub-second skew. The fix is to stop comparing a live count with a snapshot at all: persist `local_remaining` **alongside** `server_remaining` at reconcile time (one new column, written by `RecordServerLedgerReading`) and have the gauge subtract two values that were both true at the same instant. Deferred rather than rushed because it needs a migration and a redefinition of a Gate 1 metric, and folding it into an already-green 20.3 regression would mean re-running everything on a change that cannot be verified under sustained load in the same session |
| **Gate 2 trigger row 6, "Corporation / alliance departure", has no producer** | `internal/sync` contains ZERO references to `internal/provisioning` (`grep -rln provisioning internal/sync` is empty). `handlers.SyncCharacterSheet` writes the new `app.character.corporation_id` and nothing recomputes entitlements, so a character leaving the corporation that granted their Discord roles keeps them until the next bulk reconcile. Not reachable through the RBAC hook either — the change is an ESI sync write, not an RBAC mutation | **NOT fixed. Recorded for 20.4.** The fix is a package-level seam like `rbac.PermissionsChangedHook` (internal/sync must not import internal/provisioning), which is design work rather than wiring, and 20.3's scope was B27/B32/B35 plus the metric |
| **Gate 2 trigger row 8, "Admin platform lockdown", enqueues nothing** | `lockdownHandler` calls `SetPlatformLockdown` and returns. §2.3 marks the row "✓ must enqueue urgent" | **NOT fixed, and NOT because it was missed.** Lockdown FREEZES outbound provisioning; enqueueing revocations that the freeze would then refuse to perform may be exactly wrong, or may be precisely the point (freeze = deny everything). The specification does not say which, and guessing on a security control is worse than recording it. `deps.Urgent` now exists in the API process, so whichever reading 20.4 settles on is a small change. **Recorded for 20.4 as a specification question, not a defect to be silently patched** |
...
| **`make bench-ledger-clustered` fails on this machine before any 20.3 change** | 8 consecutive runs at commit `88381d6` in a clean worktree: p99 = 10.13, 10.33, 10.89, 10.68, 11.16, 12.50, 10.27, 10.69 ms against a 10 ms budget. Throughput 6867 ops/s against a 2000 ops/s target | **NOT fixed — not attributable to 20.3.** The matched 8-sample run on the fixed tree is marginally *faster* (median 10.478 vs 10.687 ms). Recorded as a blocker to be re-measured on a Linux CI runner, since Docker Desktop for Windows port-forwarding tail latency is the obvious suspect and this box cannot distinguish it from a real ceiling |
