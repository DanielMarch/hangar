...
| :-- | :-- |
| Release | `v1.0.0-rc3` |
| Started | 2026-08-21T11:00:13Z |
| Finished | 2026-08-21T15:00:13Z |
| Duration | 4h0m0s |
| Characters | 5000 |
...
| 1.3 | Ledger divergence == 0 tokens after reconciliation, per group | **pass** | max(esi_ledger_divergence) = 0 (group "char-wallet") over 9905 group-samples |
| 1.3a | Ledger prediction error recorded (no threshold) | **pass** | max(esi_ledger_prediction_error) = 12 (group "char-social") over 9905 group-samples; recorded as evidence, not bounded |
| 1.4 | Proactive error-limit pause fired, and no 420 followed | **pass** | esi_error_limit_remaining reached 19 against a pause threshold of 20; esi_420_total = 0 |
| 1.5 | Failure stayed scoped | **pass** | 1132172 requests served, 1132047 of them 200 while adversarial conditions were active |
| 1.6 | No stall — throughput never drops to zero for more than one ttl_floor | **pass** | longest interval with no request reaching the proxy: 1m30s, against a ttl_floor of 5m0s, over 962 throughput samples |
| 1.6-raw | the weaker reading 1.6 used to be — whether the run served any request AT ALL. Kept so the difference stays visible | **pass** | 1132172 requests reached the proxy over 4h0m0s |
| 1.7 | Aggregate consumption respected at N>1 | **pass** | 0 proxy-side samples showed consumption above max_tokens (replicas=1) |
| 1.8 | Mode selection correct throughout | **pass** | esi_ledger_mode observed as [solo], expected "solo" throughout |
