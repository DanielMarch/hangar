...
#### Condition 1.8 — amended in Phase 21, **amendment withdrawn in Phase 22**

Condition 1.8 stands exactly as originally written. This note is kept because a gate condition
that was amended and then un-amended should be able to show both, and because the withdrawal is
the more instructive half.

*What Phase 21 amended, and why.* At N=3, with the mid-run kill and restart §1.4 requires, the
harness observed `esi_ledger_mode` as `[clustered solo]` and failed the condition. The `solo`
reading came from the **restarted** replica, in the interval between its process starting and its
first ESI request: `ratelimit.NewGovernor1` starts in `ModeSolo` optimistically and only consults
the replica registry inside `ensureMode`, which `Acquire` calls. So as written, 1.8 was
unsatisfiable by any implementation in a run that also satisfies §1.4 — and §1.4 is mandatory.
Phase 21 amended 1.8 to be measured over samples excluding that settling interval, recording the
excluded count in the evidence.

*Why the amendment is withdrawn.* **The contradiction was never between two conditions. It was a
defect in the gauge**, catalogued as B-10 and fixed in Phase 22: `esi_ledger_mode` now emits **no
sample at all** until the replica registry has actually been read, which is the same correction
20.4.1 made for `esi_ledger_divergence` reporting 0 over zero samples — give "not measured" a
representation distinct from "measured", and here that representation is silence. No request was
ever served in the wrong mode (`Acquire` calls `ensureMode` before choosing a ledger, and on the
first call the 2-second throttle cannot skip it, because `lastModeCheck` is the zero time), so
nothing about the ledger changed; the gauge simply stopped publishing an assumption in the shape of
a reading.

With that fixed there is no settling interval to exclude, because there are no samples in it.
Condition 1.8 is satisfiable as written, and `tools/gate1-load`'s exclusion logic and its `1.8-raw`
companion reading are removed along with the amendment they existed to justify.

*The general lesson, since this is the second time.* Both 1.3 and 1.8 appeared to be contradictory
conditions and both turned out to be instrumentation that could not distinguish "no reading" from
"a reading of the default". Before amending a gate condition, check whether the metric can say "I
don't know".
