...
| # | Defect | Blocks |
| :--- | :--- | :--- |
| **B24** | §4.9 has no configuration surface — `crypto.SealWebhookSecret`/`NewWebhookSecret` have no caller, so an endpoint can only be created by hand-sealing a secret in SQL. | **FIXED in Phase 20.5.** `internal/api/v1/webhooks.go` mounts create / list / rotate / revoke plus the event-type vocabulary behind the existing `webhooks.manage` permission. The secret is returned exactly once, by the call that mints it, and by no read endpoint ever. **A rotation is an overlap, not a swap** (migration 00044): the superseded secret keeps verifying for 24h and every delivery in that window carries two `v1=` signatures, so an event already in the outbox when the rotation happens is neither dropped, delayed nor re-signed. Verified live against a real `rbac.user_role.revoked` delivery, both signatures accepted by the shipped reference verifier. | Gate 5 |
...
| **B30** | **Six ESI route handlers are never dispatched** (calendar detail and attendees, market history and prices, planet colony detail, corporation project contributors) — 13 functions with their parsers. | **FIXED in Phase 20.5, and the diagnosis was wrong in an instructive way.** The handlers were not un-dispatched: each had a `case` in its worker. They were UN-SUBSCRIBABLE — `worker.SubscribableRoutes` excluded every fan-out path, so `internal/sync/subscribe` could never create a subscription naming one and the `case` was unreachable on every installation. `fanoutRoutes()` closes it, and takes mail bodies, contract items/bids, per-division corporation wallets and the starbase detail route §4.4 names for `corporation.starbase.fuel_low` with it. Twelve of the thirteen symbols run behind six now-subscribable routes; the thirteenth, `ParseAssetNames`, is a POST with a request body and runs as an enrichment of the assets sync (there is nothing to poll a POST for). Measured live: 70 to 85 enabled subscriptions, `/markets/prices` landing 15,820 rows. | Gate 4.2 |
...
| **B33** | `$filter` validation (`filters.New`/`Validate`) has no caller. | **FIXED in Phase 20.5.** Measured before: `?standing=-10` and `?$filter=standing lt 0` both returned **200 with the whole collection** — a filter silently dropped, which returns MORE data than was asked for. Now 422 with the field named, enforced on every operation by `api.ValidateQueryFilters` against a closed set `filters.SpecFromQueryTags` derives from the operation's own `query:` tags, so the enforced set and the documented set are one thing. 422 and not 400 because Phase 15's exit criterion states that status; APPENDIX_C §6.6's 400 is the /api/v2 shim's contract and is unchanged. | Gate 4 |
| **B34** | The Redis L2 tier is never constructed, so `docker-compose.yml`'s `cache` profile is inert. | **FIXED in Phase 20.5.** `cmd/hangar.buildL2` constructs it when `HANGAR_REDIS_URL` is set, REPLACING the Postgres L2 per §5.4's DECISION. **A Redis outage mid-request is a MISS, not a failure and not a fall-through to Postgres** — when Redis is the L2 nothing has been written to `app.esi_cache_entry`, so a fall-through would serve a body the configured tier does not contain. Proved with the tier absent, cold, warm, and terminated mid-suite: identical statuses and identical bytes in all four. | Gate 5.5 |
...
| **B37** | **HANGAR requests no ESI scopes at login.** `internal/api/v1/auth.go` calls `flow.BeginLogin(ctx, []string{}, …)` at both call sites — the login redirect and the character-reauthorize handler — so the authorization URL asks for an empty scope set. EVE SSO's own documentation is explicit that an application requesting no scopes "will only be able to authenticate users, and no refresh tokens will be issued". Every authenticated route in the sync layer therefore has no token to call with, no matter what is registered in the developer portal. Found while registering the SSO application (docs/SSO_APPLICATION.md). | Gate 1, Gate 2, and every authenticated capability |
| **B38** | **Two sync-set paths do not exist in the ESI spec.** `internal/sync/worker` registers `/characters/{character_id}/calendar/events` (spec: `/characters/{character_id}/calendar`) and `/corporations/{corporation_id}/projects/{project_id}/contributions` (spec: `…/contributors`). Both are a plural inferred from the resource name — the precise failure Principle 5's "stored verbatim, never derived or pluralised" exists to prevent. Neither can ever return data. Undetected because the handlers behind them are B30's, and are never dispatched. | **PATH FIXED in Phase 20.2; the DTO fixed in Phase 20.5.** Renaming the path left the `contributions` parser attached to the `contributors` route, and it had still never executed. Read from the live spec: `CorporationsProjectsContributors` is an OBJECT of `{contributors, cursor}` whose elements are `{id, name, contributed}`. **`id` is the contributor's CHARACTER id** and **`contributed` is int64 PROGRESS, not isk** — the project's isk field is `reward_isk`, and `contributed` is denominated in the same units as `current_progress`/`target_progress`. One response feeds both `app.corporation_project_contributor` (the roster) and `app.corporation_project_contribution` (`contributed` as `amount`), because there is one route and 00011 built two tables as if there were two. | Gate 4.2, Principle 5 |
| **B39** | **An unauthenticated `GET /` renders a blank page.** `/login` renders correctly and `web/src/routes/_authed.tsx`'s `beforeLoad` does contain the right `redirect({ to: "/login" })` on a failed `meQueryOptions` resolve — so the guard exists and does not take effect on a cold load of `/`. Compounding it, `web/src/routes/__root.tsx` declares **no `errorComponent`**, so any throw that is not a redirect renders nothing at all, with no message in the UI: the failure mode of the whole SPA is a white screen. Observed twice against a real installation, with a 401 on `/api/v1/me` in the console. A first-time visitor to the installation's root URL sees a blank page. | Gate 5 (a deployment whose front door is blank is not "a running installation") |
| **B40** | **A freshly-authenticated SSO user holds zero permissions.** Confirmed live with a real character: `GET /api/v1/me` answers 200, and every character and corporation data endpoint answers **403**. This is B26's downstream consequence made concrete — `rbac.CreateRole`, `AssignUserRole` and `AddRoleGrant` have no production caller, so no role exists, nothing assigns one, and SSO login produces a user who can see nothing. `hangar admin bootstrap-token` mints a scoped token and is the only way into a fresh installation, which makes the *interactive* login a dead end. Separately, `web/src/main.tsx` sets a global React Query `retry: 1`, so each of these 403s is retried once — doubling request volume for a failure that is definitionally not transient (`meQueryOptions` already sets `retry: false`; the default does not). | Gate 5, Gate 4.3 |
| **B42** | **The sync engine has no subscription creator — NOTHING is ever scheduled.** `UpsertSyncSubscription` is called by exactly three integration tests and by no production code, and no migration or seed inserts into `app.sync_subscription` either. So on a real installation the table is permanently empty: the planner acquires leadership, claims due work, finds none, forever; every registered River worker is never dispatched; and a character who has authorised all 46 scopes has nothing fetched on their behalf. Phases 6, 7, 8 and 9 — the entire ESI sync engine — cannot execute. Confirmed against a live installation with a real authorised character: 225 catalogued routes, 46 granted scopes, **0 subscriptions**. The dashboard's own text ("Tranquility status has not been synced yet — ESI's `/status` route **is scheduled** but has not completed a successful run") is wrong in the same way: it is not scheduled. | **FIXED in Phase 20.1.1.** `internal/sync/subscribe` reconciles `app.sync_subscription` from four set-based SQL statements, triggered from the SSO callback, from `hangar admin sync reconcile`, and from a five-minute pass in `serve`. Four decisions, each recorded where the code lives: (1) the route→entity partition is DERIVED from the three dispatch tables via `worker.SubscribableRoutes()`, never restated, and excludes the fan-out detail routes because they carry a second path parameter a single `entity_id` cannot express; (2) subscriptions are scope-gated by a `NOT EXISTS` join against `app.character_token_scope`, because a subscription that must 403 would spend Governor 2's installation-wide error budget on requests that cannot succeed; (3) revocation DISABLES rather than deletes, preserving the etag/cursor state so a restored scope resumes conditionally; (4) the bootstrap is ordered and therefore periodic — corporation subscriptions cannot exist until a character route populates `corporation_id`. **Verified end to end against real ESI**: 35 character + 3 global subscriptions on the first pass, all 38 returning HTTP 200, 53 skills and 2 corporation-history rows persisted, `corporation_id` populated to 98840805, then 32 corporation subscriptions on the second pass and a clean no-op on the third. | §6, §7 Phases 6-9 |
| **B43** | **The reachability guard had a blind spot the size of the store layer.** Phase 20.1's `make check-reachability` runs `x/tools/cmd/deadcode`, which by design does not report dead functions in **generated** files. `internal/store/gen` is generated, so all 442 query methods — the widest surface in the codebase — were invisible to the guard built specifically to catch unwired subsystems. B42 hid there. | **FIXED in the Phase 20.1 regression.** `TestEveryGeneratedQueryHasAProductionCaller` checks every method on the generated `Querier` interface against `test/reachability/generated_allowlist.txt`, with the same two-way contract as the main allowlist. Baseline: **72 of 442 have no production caller**, each recorded against the defect it belongs to. | — |
| **B44** | **The acting-character election could never succeed — a bootstrap deadlock.** `internal/sync.DBElector`'s only candidate pool is `app.corporation_member`, and that table's only writer was `SyncCorporationMembers` — a CORPORATION-scoped route, which cannot run until a character has been elected. Election needed members; members needed election. No corporation route could ever run on any installation. | **FIXED in Phase 20.1.1.** `SyncCharacterSheet` now records the character's own membership: ESI has just told us authoritatively which corporation this character belongs to, and one member is all the elector needs. `SyncCorporationMembers` still owns the full roster once a corporation route can run. | §6.3 |
| **B45** | **The elector read a table nothing writes.** It resolved corporation roles through `ListCorporationRoles` against `app.corporation_role`, which has **no production writer anywhere** and is permanently empty — so `hasRoles` was false for every candidate on every route, and no election could have succeeded even with a populated pool. The roles ESI actually returns arrive via `GET /characters/{character_id}/roles`, a character-scoped route that does run, and land in `app.character_role` (216 rows for the first real character authorised). | **FIXED in Phase 20.1.1** — the elector reads `app.character_role`. | §6.3 |
| **B46** | **A CEO or Director was refused routes they can perform in game.** EVE's corporation roles are hierarchical and ESI reports them literally: the CEO holds every role and cannot be stripped of any, and a Director likewise holds all roles implicitly, so ESI never enumerates the subordinate roles for either. A literal set-membership test therefore locked out the single most privileged character in a corporation — usually the only one who can serve the route at all. | **FIXED in Phase 20.1.1.** `satisfiesRoles` treats `app.corporation.ceo_id` and the `Director` role as satisfying any requirement. Found by a real CEO's token: CEODude holds `Director` and was being refused all 32 corporation routes. | §6.3 |
| **B47** | **HANGAR polled every route exactly ONCE, then not again for 24 hours.** `planner.SyncJobArgs.InsertOpts` set `river.UniqueOpts{ByArgs: true}` and omitted `ByState`. River's default for `ByState` **includes `JobStateCompleted`**, and its documentation is explicit: a completed unique job still blocks a duplicate insert "at least not until the job cleaner maintenance process eventually removes the completed job" — `CompletedJobRetentionPeriodDefault` is **24 hours**. So §6.2's entire scheduling policy (`x-cache-age`, the TTL floor, the 1.5^n adaptive backoff, all of `PlanNextDueAt`) was silently overridden by a job-cleaner interval: a route declaring a 300-second cache age was polled once per day. Measured before the fix — the planner claimed 70 due subscriptions and River inserted **zero** jobs. | **FIXED in Phase 20.1.1** by setting `ByState` to the four states River requires plus `Retryable`, excluding `Completed`/`Cancelled`/`Discarded`. Uniqueness still prevents concurrent duplicate work (§6.1's stated intent) and no longer prevents all future work. Verified: `river_job` 70 → 102 on the next cycle. | §6.1, §6.2 |
| **B48** | **`/corporations/{id}/members` died on the first member who is not a HANGAR user.** The route returns bare character IDs for every member; `SyncCorporationMembers` upserted `corporation_member` directly, and `corporation_member_character_id_fkey` rejected every id with no `app.character` row (SQLSTATE 23503). | **FIXED in Phase 20.1.1** — `UpsertCharacterStub` creates the minimal row first, with an empty name rather than an invented one, mirroring `UpsertCorporationStub`. | §6 |
| **B49** | **`/corporations/{id}/projects` was parsed as the wrong JSON shape.** The parser expected a bare array; the operation's 200 response is `CorporationsProjectsListing`, an **object** `{cursor, projects}`. Every attempt failed with "cannot unmarshal object into Go value of type []handlers.CorporationProjectDTO". The hand-written golden fixture encoded the same wrong assumption, so no test caught it. | **FIXED in Phase 20.1.1** — the envelope is parsed and the fixture corrected to the spec. `cursor` is captured but not followed: §5.9's cursor mechanism has no live implementation (defect B31), so a corporation with more than one page of projects currently syncs only the first, recorded rather than silently truncated. | §5.9, §6 |
| **B41** | **`serve`'s startup catalogue ingest overwrites a seeded `esi.d_max`, and it is a race.** `catalogue.Boot` is the only writer of `app.setting`'s `esi.d_max` and rewrites it on every startup from the live spec, or from the embedded snapshot on fallback. Any environment that has deliberately seeded a catalogue and a D_max ceiling therefore has that seed replaced moments after the process comes up. `web/e2e/global-setup.ts` seeds `esi.d_max = 2026-08-11` plus a five-route catalogue so the pin-advance preview has a deterministic diff; the ingest replaced the ceiling with the snapshot's `2026-08-04` and two `pin-advance.spec.ts` cases failed. **It survived the Phase 19 close-out because it is a race**: the seed wins on a machine that cannot reach ESI and loses on one that can, so `make ci-strict` passed where ESI was unreachable and failed here. Introduced by `a4c0400` (the B20 fix), which added the background ingest without considering the seeded-environment case. | **FIXED in Phase 20.1's regression.** `HANGAR_ESI_STARTUP_CATALOGUE_INGEST` (default `true`, preserving §2's single-process default) makes the ingest opt-out-able; `playwright.config.ts` sets it `false`. It is a real operator setting as well as a test control — an installation managing its catalogue on a schedule, or running air-gapped, should not have every replica restart reach for ESI. | §5.1, Gate 5 |
